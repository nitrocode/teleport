...
## Step 1. Enable a JWT realm in Elasticsearch
...
- Set `allowed_audiences` to the address which Teleport Application Service
  will use to connect to Elasticsearch (the `uri` field in the application YAML
  configuration).
...
## Step 2. Register an Elasticsearch application in Teleport
...
## Step 3. Connect to the ElasticSearch API
